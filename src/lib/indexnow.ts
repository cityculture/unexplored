...
const BASE_URL = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.cityculture.in';
...
export async function fetchAllSitemapUrls(): Promise<string[]> {
  ...
  try {
    ...
    for (const sitemapPath of sitemaps) {
      const response = await fetch(`${BASE_URL}${sitemapPath}`);
      if (response.ok) {
        const text = await response.text();
        const sitemapUrls = extractUrlsFromXml(text);
